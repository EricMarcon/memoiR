...
% Additional packages
\RequirePackage{natbib}             % Advanced Bibliography (citep...). Before babel.
\RequirePackage[utf8]{inputenc}     % For accents
\RequirePackage[T1]{fontenc}        % For accents
...
\usepackage{caption}                % After babel
...
\RequirePackage[french, english]{babel}
  \frenchbsetup{StandardLists=true} % Necessary with \usepackage[french]{babel} or bullets are hyphens
...
{\centering \includegraphics[width=0.8\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/RtmpRSTZfp/simple_article/gallery/simple_article/simple_article_files/figure-latex/pressure-1} 
...
Lists are indicated by \emph{, + and - (three hierarchical levels) or numbers 1., i. and A. (numbered lists).
...
Bibliographic references included in the \texttt{references.bib} file can be called by \texttt{{[}@CitationKey{]}}, in parentheses \citep{Xie2016}, or without suare brackets, in the text, as \citet{Xie2018} .
...
The style is included in the template: it is the one from \emph{Methods in Ecology and Evolution}.
...
\tightlist
\item
  To obtain two-column tables by redefining the \texttt{table} environment and loading the \emph{tabu} package necessary for \emph{kableExtra}:
\end{itemize}
