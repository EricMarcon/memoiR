...
Markdown allows to produce, without rewriting the text, a document in different formats: article LaTeX or Word for example.
...
The EcoFoG package templates are displayed: choose Article EcoFoG.

Write the document in RStudio.

Clicking the \textbf{Knit} button in RStudio generates the document in the requested format.

It is better to create a RStudio project to benefit from all the possibilities: \emph{File} / \emph{New Project} then use the wizard to create a project from an existing folder.
...
The horizontal \texttt{-} and vertical separators \texttt{\textbar{}} allow you to draw an array according to Markdown syntax, but this is not the best method.
...
The caption is specified by the \texttt{caption} argument and referencing is possible because the array receives a label whose name is \texttt{tab:} followed by the name of the code snippet (array \ref{tab:kable}).
...
In LaTeX, tables can have the width of the column and possibly span multiple pages, or use the width of the page, like the table \ref{tab:Paracou}).
...
A knitr bug causes the table format (``html'') to be passed incorrectly to \texttt{kable\_styling} when knitting to gitbook format, which generates a warning and prevents the option from being taken into account.
...
{\centering \includegraphics[width=0.8\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/RtmpAEqK2T/stylish_article/gallery/stylish_article/stylish_article_files/figure-latex/pressure-1} 
...
Cross-references are made with the command \texttt{\ref{fig:xxx}}.
...
\caption{New document}\label{fig:logo}
\end{figure}
...
They can be numbered, see equation \eqref{eq:disque}, using the \texttt{equation} environment:
...
In all cases, the call to the reference is made by the command \texttt{ref(ref:zzz)}.

\hypertarget{bibliography}{%
\subsection{Bibliography}\label{bibliography}}

Bibliographic references included in the references.bib file can be called in the text, in parentheses \citep{Xie2016}, or in the text, as \citet{Xie2018}.
...
The style is included in the EcoFoG template: it is the one from \emph{Methods in Ecology and Evolution}.
It cannot be changed, to ensure the homogeneity of the documents produced.

To prepare the submission of a manuscript to a journal, it will be necessary to open the intermediate \emph{.tex} file produced by Pandoc and to copy the contents of the environment into the template proposed by the journal, which will take care of the formatting.
...
\tightlist
\item
  to obtain two-column arrays by redefining the \texttt{table} environment and loading the \emph{tabu} package necessary for \emph{kableExtra}:
\end{itemize}
...
\item
  complex commands (such as \texttt{renewenvironment}) must be entered on a single line otherwise they will be destroyed by knitr at the first knitting in HTML.
\end{itemize}
...
LaTeX templates use the LaTeX package \emph{babel} which recognizes the language names \emph{English} and \emph{french}.
...
\hypertarget{optimization-for-github}{%
\subsection{Optimization for GitHub}\label{optimization-for-github}}

The draft article can be deposited on Github\footnote{\url{https://ericmarcon.github.io/travailleR/chap-git.html}}.

The provided R script, \emph{GithubPages.R}, makes it easy to publish the article to the web pages associated with the repository by moving the HTML files produced by knitting the article into the \emph{docs} folder.

Then you just have to activate the web pages of the repository, adapt the \emph{README.md} file provided (follow its instructions) and duplicate it in the \emph{docs} folder.

The web pages can be produced by continuous integration\footnote{\url{https://ericmarcon.github.io/travailleR/chap-ci.html}}.

\hypertarget{other-templates}{%
\subsection{Other Templates}\label{other-templates}}

The EcoFoG memo template is simpler, formatted on a single column.

The \emph{rticle} package provides templates for articles (PLOS, PNAS, etc.), but requires the use of LaTeX commands and does not allow knitting in HTML.
The \emph{xaringan} package provides an HTML 5 presentation template.

The \emph{Book} template of the EcoFoG package allows to write books.

The last line of the template (R code snippet) must be kept to display the title \emph{References} (to be translated into the document language if necessary) in HTML format.
