...
\leavevmode\vadjust pre{\hypertarget{pythkao}{}}%
...
Enter it in a paragraph starting with \texttt{(ref:pythkao)} followed by a space.
Then, call the text with the code \texttt{(ref:pythkao)}.
...
\item
  In \texttt{latex/preamble.tex}, define the color by adding the following line below that defining the grey color:
\end{itemize}
