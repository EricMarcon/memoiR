...
A [gallery](https://mirrors.ctan.org/info/latex-samples/MemoirChapStyles/MemoirChapStyles.pdf) is available[^1].
...
If **tinytex** installed it, the packages can be installed by R:
```{r, eval=FALSE}
...
Note that long tables (`longtable = TRUE` argument in the `kable()` function to render tables) may not be cut correctly among pages when specific fonts are declared.

The font size is 11 by default.
Increase it if necessary, depending on the font type, the margin width and personal preferences.


# Conclusion

Large margins are optimal with the `companion` style and the `verbose-inote` bibliographic style; small margins should be used the default options.
