...
````
```{r iris, echo=FALSE, warning=FALSE}`r ''`
...
````
```{r Paracou, echo=FALSE, warning=FALSE}`r ''`
...
names(Intervention)[5] <- "\\%AGB lost"
knitr::kable(Intervention, caption="Summary of the disturbance intensity for the 4 plot treatments in Paracou.", longtable = FALSE, booktabs = TRUE, escape = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = TRUE)
```
...
````
```{r pressure, fig.cap="Figure title"}`r ''`
plot(pressure)
```
````

```{r pressure, fig.cap="Figure title", echo=FALSE}
plot(pressure)
```
...
````
```{r cover, fig.cap="A figure from a file", echo=FALSE, out.width='60%'}`r ''`
...
```{r ChunkName, fig.cap="(ref:ChunkName)"}`r ''`
plot(pressure)
```
````
