...
Rely on the following functions to simplify that task:

- `build_gitignore()` to create a `.gitignore` file. Activate git source control then.
- `build_index()` to create a `README.md` file, that will be the GitHub Pages index.
- `build-githubpages()` to move knitted documents (articles or slideshows) to the `docs` folder.


# Credits

The feather in the memoiR package sticker is from [Wikimedia](https://fr.wikipedia.org/wiki/Fichier:Plume.svg#/media/Fichier:Plume.svg) under licence[CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/).
