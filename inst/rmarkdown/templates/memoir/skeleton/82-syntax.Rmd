...
(ref:pythkao) Pythagorean theorem

:::{#pythkao .greybox data-latex='[frametitle=(ref:pythkao)]' title='(ref:pythkao)'}
...
Arguments `data-latex` and `title` contain the title of the box in the PDF and HTML outputs, so a referenced text is useful to avoid repeating it.
Note the particular syntax of the title in the PDF output.

Grey text boxes (`.greybox`) are allowed by default.
...
- In `latex/preamble.tex`, define the color by adding the following line below that defining the grey color:
```
